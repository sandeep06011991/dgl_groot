...
public:
  ...
  template <typename T> void Allgather(std::vector<T> &vec) {
  ...
  template <typename T> T Scatter(const std::vector<T> &vec) {
    if (is_root_) {
      CHECK(vec.size() == n_peers_);
      for (int i = 0; i < n_peers_; ++i) {
        auto bs = std::make_shared<BinStream>();
  ...
  template <typename T> std::vector<T> Gather(const T &val) {
    auto bs = std::make_shared<BinStream>();
  ...
  template <typename T> T RingExchange(int needed_peer_id, const T &val) {
    auto bs = std::make_shared<BinStream>();
