...
template <typename T> void DSCudaMalloc(T **ptr) { cudaMalloc(ptr, sizeof(T)); }

template <typename T> void DSCudaMalloc(T **ptr, int size) {
  cudaMalloc(ptr, sizeof(T) * size);
  cudaMemset(*ptr, 0, sizeof(T) * size);
}

template <typename T> void DSMallocAndCopy(T **ret, const T *src, int size) {
  DSCudaMalloc(ret, size);
  cudaMemcpy(*ret, src, sizeof(T) * size, cudaMemcpyHostToDevice);
}

template <typename T> void DSMallocAndCopy(T **ret, const std::vector<T> &src) {
  DSMallocAndCopy(ret, src.data(), src.size());
}

template <typename T> std::vector<T> DSDeviceVecToHost(T *ptr, size_t size) {
...
template <typename T> T GetEnvParam(const std::string &key, T default_value) {
...
template <typename T> T GetEnvParam(const char *str, T default_value) {
  return GetEnvParam<T>(std::string(str), default_value);
}

template <typename T> void SetEnvParam(const std::string &key, T value) {
  auto new_key = std::string("DGL_DS_") + key;
  setenv(new_key.c_str(), std::to_string(value).c_str(), 1);
}
template <typename T> void SetEnvParam(const char *key, T value) {
...
/**
 * @brief Create an IdArray on shared memory. The memory is created based on the
 * array on the root process. However, other processes must have the correct
...
static void Register(IdArray array) {
  ...
  assert(data != nullptr);
  if (size > 0) {
    CUDACHECK(cudaHostRegister((void *)data, sizeof(uint64_t) * size,
