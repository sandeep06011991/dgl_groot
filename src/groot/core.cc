...
// where sampler and trainers have to coordinated
// I do not need this at this moment.
// #include "schedule.h"
...
//      new Coordinator(rank, world_size, comm_port));
  ...
  for (int i = 0; i < N_PINNED_ARRAY; ++i) {
    ds_thread_local->pinned_array[i] = CreatePinnedArray(
        DLDataType({kDLInt, 64, 1}), THREAD_LOCAL_PINNED_ARRAY_SIZE);
  }
}
