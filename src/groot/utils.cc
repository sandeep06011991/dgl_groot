...
IdArray CreateShmArray(IdArray arr, const std::string& shm_name) {
  ...
  if((arr->dtype.code == DGLDataTypeCode::kDGLInt) && (arr->dtype.bits == 32)){
    dl_tensor.dtype = DLDataType{.code = DGLDataTypeCode::kDGLInt,\
                  .bits = 32, .lanes = 1};
  }else{
...
IdArray CreatePinnedArray(DLDataType dtype, size_t size) {
  auto ret = IdArray::Empty({(signed long)size}, \
      (DGLDataType{.code = dtype.code,.bits = dtype.bits, .lanes = dtype.lanes  }) , 
