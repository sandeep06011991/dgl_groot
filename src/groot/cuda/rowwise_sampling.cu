...
void CSRRowWiseSamplingUniform(NDArray indptr, NDArray indices, NDArray rows,
...
//  IdType *const out_deg = static_cast<IdType *>(block->_outdeg->data);
  if (replace) {
    ...
    CUDA_KERNEL_CALL(impl::_CSRRowWiseSampleDegreeReplaceKernel, grid, block, 0,
                     stream, num_picks, num_rows, slice_rows, in_ptr, out_deg);
  } else {
    const dim3 block(512);
    const dim3 grid((num_rows + block.x - 1) / block.x);
    CUDA_KERNEL_CALL(impl::_CSRRowWiseSampleDegreeKernel, grid, block, 0,
                     stream, num_picks, num_rows, slice_rows, in_ptr, out_deg);
  }
...
//  NDArray &new_len_tensor = block->_new_len_tensor;
  ...
  if (replace) { // with replacement
    ...
    CUDA_KERNEL_CALL(
        (impl::_CSRRowWiseSampleUniformReplaceKernel<IdType, TILE_SIZE>), grid,
        block, 0, stream, random_seed, num_picks, num_rows, slice_rows, in_ptr,
        in_cols, data, out_ptr, out_rows, out_cols, out_idxs);
  } else { // without replacement
    const dim3 block(BLOCK_SIZE);
    const dim3 grid((num_rows + TILE_SIZE - 1) / TILE_SIZE);
    CUDA_KERNEL_CALL((impl::_CSRRowWiseSampleUniformKernel<IdType, TILE_SIZE>),
                     grid, block, 0, stream, random_seed, num_picks, num_rows,
                     slice_rows, in_ptr, in_cols, data, out_ptr, out_rows,
                     out_cols, out_idxs);
  }
