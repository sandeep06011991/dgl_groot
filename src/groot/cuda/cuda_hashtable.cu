...
namespace dgl {
    namespace groot {
        ...
        inline T RoundUpDiv(T target, D unit) {
        ...
        public:
            ...
            inline __device__ bool AttemptInsertAtO2N(
                    const IdType pos, const IdType id, const IdType index,
            ...
            inline __device__ IteratorO2N
            InsertO2N(const IdType id, const IdType index, const IdType version) {
            ...
            inline __device__ IteratorN2O
            InsertN2O(const IdType pos, const IdType global) {
 ...
 */
        ...
        __global__ void generate_hashmap_duplicates(
                ...
                MutableDeviceOrderedHashTable<IdType> table, const IdType version) {
...
#pragma unroll
        ...
        __global__ void generate_hashmap_unique(
                const IdType *const items, const size_t num_items,
                MutableDeviceOrderedHashTable<IdType> table, const IdType global_offset,
                const IdType version) {
            assert(BLOCK_SIZE == blockDim.x);
...
#pragma unroll
        ...
        __global__ void compact_hashmap(
                ...
                const IdType *const num_items_prefix, IdType *const num_unique_items,
        ...
        __global__ void compact_hashmap_revised(
                const IdType *const items, const size_t num_items,
                MutableDeviceOrderedHashTable<IdType> table,
                const IdType *const num_items_prefix, const IdType global_offset,
                const IdType version) {
            assert(BLOCK_SIZE == blockDim.x);
        ...
        __global__ void compact_hashmap_revised_mutable(
                IdType *items_pos, const size_t num_items,
                MutableDeviceOrderedHashTable<IdType> table,
                const IdType *const num_items_prefix, const IdType global_offset,
                const IdType version) {
            assert(BLOCK_SIZE == blockDim.x);
...
// DeviceOrderedHashTable<IdType> implementation
        ...
        DeviceOrderedHashTable<IdType> OrderedHashTable<IdType>::DeviceHandle() const {
            return DeviceOrderedHashTable<IdType>(
                    _o2n_table, _n2o_table, _o2n_size, _n2o_size);
        }

// OrderedHashTable<IdType> implementation
        template<typename IdType>
        OrderedHashTable<IdType>::OrderedHashTable(
                const size_t size, DGLContext ctx, const size_t scale)
                : _o2n_table(nullptr),
        ...
        void OrderedHashTable<IdType>::FillWithDuplicates(
            ...
            generate_hashmap_duplicates<
                    IdType, Constant::kCudaBlockSize, Constant::kCudaTileSize>
            <<<grid, block, 0, cu_stream>>>(input, num_input, device_table, _version);
            device->StreamSync(_ctx, stream);

            // LOG(INFO) << "OrderedHashTable<IdType>::FillWithDuplicates generate_hashmap_duplicates with " << num_input << " inputs";
            ...
            <<<grid, block, 0, cu_stream>>>(
                    input, num_input, device_table, item_prefix, gpu_num_unique,
                    _num_items, _version);
            device->StreamSync(_ctx, stream);

            device->CopyDataFromTo(
                    gpu_num_unique, 0, num_unique, 0, sizeof(IdType), _ctx,
        ...
        template<typename IdType>
        void OrderedHashTable<IdType>::CopyUnique(
                IdType *const unique, cudaStream_t stream) {
            auto device = runtime::DeviceAPI::Get(_ctx);
            device->CopyDataFromTo(
                    _n2o_table, 0, unique, 0, sizeof(IdType) * _num_items, _ctx, _ctx,
                    DGLDataTypeTraits<IdType>::dtype);
            device->StreamSync(_ctx, stream);
        }
...
//  auto stream = static_cast<DGLStreamHandle>(cu_stream);
            ...
            CUDA_CALL(
                    cudaMemcpyAsync(new_len_tensor->data, item_prefix + grid.x, sizeof(IdType), cudaMemcpyDeviceToHost,
                                    cu_stream));
...
//  }
        ...
        template<typename IdType>
        void OrderedHashTable<IdType>::FillWithDupMutable(
                IdType *input, const size_t num_input, cudaStream_t stream) {
            if (num_input == 0) return;
            const size_t num_tiles = RoundUpDiv(num_input, Constant::kCudaTileSize);
            const dim3 grid(num_tiles);
            const dim3 block(Constant::kCudaBlockSize);
            ...
            // 3. now each block knows where in n2o to put the node
            compact_hashmap_revised_mutable<
                    IdType, Constant::kCudaBlockSize, Constant::kCudaTileSize>
            <<<grid, block, 0, cu_stream>>>(
                    input, num_input, device_table, item_prefix, _num_items, _version);
            device->StreamSync(_ctx, stream);
            IdType tmp;
            device->CopyDataFromTo(
                    item_prefix + grid.x, 0, &tmp, 0, sizeof(IdType), _ctx,
        ...
        template<typename IdType>
        void OrderedHashTable<IdType>::FillNeighbours(
                const IdType *const indptr, const IdType *const indices,
                cudaStream_t stream) {
            ...
            gen_count_hashmap_neighbour_single_loop<
                    IdType, Constant::kCudaBlockSize, Constant::kCudaTileSize>
            <<<grid, block, 0, cu_stream>>>(
                    input, num_input, indptr, indices, device_table, item_prefix, nullptr,
                    _version);
            device->StreamSync(_ctx, stream);
            ...
            // 3.now each block knows where in n2o to put the node
            compact_hashmap_neighbour_single_loop<
                    IdType, Constant::kCudaBlockSize, Constant::kCudaTileSize>
            <<<grid, block, 0, cu_stream>>>(
                    input, num_input, indptr, indices, device_table, item_prefix, nullptr,
            ...
            if (num_input != 0) {
                device->CopyDataFromTo(
                        gpu_num_unique, 0, &_num_items, 0, sizeof(IdType), _ctx,
                        DGLContext{kDGLCPU, 0}, DGLDataTypeTraits<IdType>::dtype);
                device->StreamSync(_ctx, stream);
            }
        ...
        void OrderedHashTable<IdType>::FillWithUnique(
            ...
            generate_hashmap_unique<
                    IdType, Constant::kCudaBlockSize, Constant::kCudaTileSize>
            <<<grid, block, 0, cu_stream>>>(
                    input, num_input, device_table, _num_items, _version);
            runtime::DeviceAPI::Get(_ctx)->StreamSync(_ctx, stream);
        ...
        template
        class OrderedHashTable<int32_t>;

        template
        class OrderedHashTable<int64_t>;
    }  // namespace groot
}  // namespace dgl