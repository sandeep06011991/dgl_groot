...
namespace dgl {
    namespace groot {
        namespace impl {
            ...
            FindUVAKernelConfig(int feat_len, int feat_width, int feat_bytes = 4, int CUDA_MAX_THREAD_NUM = 1024) {
            ...
            template<typename DType, typename IdType>
            __global__ void IndexSelectSingleKernel(
                    const DType *array, const IdType *in_index, const IdType *out_index, const int64_t length,
                    const int64_t arr_len, DType *out) {
                int tx = blockIdx.x * blockDim.x + threadIdx.x;
                int stride_x = gridDim.x * blockDim.x;
                while (tx < length) {
            ...
            template<typename DType, typename IdType>
            __global__ void IndexSelectMultiKernel(
                    const DType *const array, const int64_t num_feat,
                    const IdType *const in_index, const IdType *const out_index,
                    const int64_t length, const int64_t arr_len, DType *const out) {
        ...
        template<typename DType, typename IdType>
        void _IndexSelect(NDArray array, IdArray index, NDArray &ret, cudaStream_t stream) {
            bool is_pinned = array.IsPinned();
            const int64_t arr_len = array->shape[0];
            const int64_t len = index->shape[0];
            int64_t num_feat = 1;
...
//                shape.emplace_back(array->shape[d]);
            ...
            ret->shape[0] = len;
            // use index->ctx for pinned array
            if (len == 0 || arr_len * num_feat == 0) return;
            DType *ret_data = static_cast<DType *>(ret->data);
        ...
        template<typename DType, typename IdType>
        void _IndexSelect(NDArray array, IdArray in_index, IdArray out_index, NDArray &ret, cudaStream_t stream) {
            bool is_pinned = array.IsPinned();
            const int64_t arr_len = array->shape[0];
...
//                shape.emplace_back(array->shape[d]);
            ...
            if (num_feat == 1) {
                ...
                const int nb = (len + nt - 1) / nt;
                CUDA_KERNEL_CALL(
                        impl::IndexSelectSingleKernel, nb, nt, 0, stream, array_data, in_index_data, out_index_data, len,
                        arr_len, ret_data);
            } else if (!is_pinned) {
                dim3 block(256, 1);
                while (static_cast<int64_t>(block.x) >= 2 * num_feat) {
                ...
                CUDA_KERNEL_CALL(
                        impl::IndexSelectMultiKernel, grid, block, 0, stream, array_data, num_feat,
                        in_index_data, out_index_data, len, arr_len, ret_data);
            } else {
                // array is pinned
                const auto config = impl::FindUVAKernelConfig(len, num_feat, array->dtype.bits / 8);
                const auto grid = config.first;
        ...
        template void _IndexSelect<int8_t, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int8_t, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int16_t, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int16_t, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int32_t, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int32_t, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int64_t, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<int64_t, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<__half, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<__half, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

#if BF16_ENABLED

        template void _IndexSelect<__nv_bfloat16, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<__nv_bfloat16, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

#endif  // BF16_ENABLED

        template void _IndexSelect<float, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<float, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<double, int32_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        template void _IndexSelect<double, int64_t>(
                NDArray, IdArray, NDArray &, cudaStream_t stream);

        // with buff and out index

        template void _IndexSelect<int8_t, int32_t>(
                NDArray, IdArray, IdArray, NDArray &, cudaStream_t stream);
        ...
        void IndexSelect(NDArray array, IdArray in_index, IdArray out_index, NDArray &out_buff, cudaStream_t stream){
            std::string header = "IndexSelect";
            CHECK_EQ(in_index->shape[0], out_index->shape[0]) << "in_index and out_index must have the same shape";
            ...
            ATEN_DTYPE_BITS_ONLY_SWITCH(array->dtype, DType, "values", {
                ATEN_ID_TYPE_SWITCH(in_index->dtype, IdType, {
                        return _IndexSelect<DType, IdType>(array, in_index, out_index, out_buff, stream);
                });
            });
        };
    } // groot
