...
//  auto root_conn_addr =
//      std::string("tcp://") + GetHostName() + ":" + std::to_string(master_port);
//  if (is_root_) {
//    root_receiver_ =
//        std::unique_ptr<zmq::socket_t>(new zmq::socket_t(*zmq_ctx_, ZMQ_PULL));
...
//      LOG(INFO) << "Get the info of peer " + std::to_string(peer_rank) +
//                       " on address " + peer_addr;
//      CHECK_LT(peer_rank, n_peers_);
//
//      to_peers_[peer_rank] = std::unique_ptr<zmq::socket_t>(
//          new zmq::socket_t(*zmq_ctx_, ZMQ_PUSH));
...
int Coordinator::RootRecvInt() {
//    return zmq_recv_int32(root_receiver_.get());
}
