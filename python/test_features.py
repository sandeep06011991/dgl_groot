...
def test_feature_basic():
    ...
    print(feature_order, "feature order")
    ...
    print("distributed fetch latency", e1.elapsed_time(e2)/1000)
    print(
        f"Process {os.getpid()}: TEST SUCCEED!, With Memory Bandwidth = { res.cpu().numpy().size * 4 / consumed_time / 1024 / 1024 / 1024} GB/s, consumed {consumed_time}s"
    )
    local_limit = int(.9 * (1024  ** 3)// (FEATURE_DIM * 4))
    print("local limit", local_limit, host_tensor.shape)
    local_indices = torch.randint(0,local_limit, device_indices.shape, dtype = torch.long).to(rank)
    start = time.time()
    e1.record()
    res1= feature[local_indices]
    e2.record()
    e2.synchronize()
    print("local fetch", e1.elapsed_time(e2)/1000)
    consumed_time = e1.elapsed_time(e2)/1000
    res = res.cpu().numpy()
