...
def train_cache(rank: int, config: RunConfig, dgl_dataset: DGLDataset):
...
def groot_cache(config: RunConfig):
    ...
    if config.world_size == 1:
        train_cache(config.rank, config, dgl_dataset)
    else:
        mp.spawn(train_cache, args=(config, dgl_dataset), nprocs=config.world_size, daemon=True)# def groot_cache(config: RunConfig):
