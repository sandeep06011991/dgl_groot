...
def best_configuration():
    for graph_name in ["ogbn-products", "ogbn-papers100M"]:
        configs = get_number_of_redundant_layers(graph_name = graph_name,\
               system = "groot", log_path='./log/redundant_layers.csv', data_dir=get_data_dir(graph_name))
...
def quiver_experiment(graph_name: str):
        ...
        #                          data_dir=get_data_dir(graph_name))
        # configs.append(config)
        config = get_batchsize_config(graph_name= graph_name, system="quiver", log_path="./log/batch_size.csv", data_dir=get_data_dir(graph_name))
        configs.append(config)
        ...
        for partitioning_graph in ["","pruned"]:
            for balancing in ["edge","samp"]:
                for training_nodes in ["bal","xbal"]:
                    ...
                    configs.append(config)
        test_acc = "ogbn" in graph_name
        bench_groot_batch(configs, test_acc = test_acc, try_caching= True)
